      ...
      <div class="card">
        <div class="card-body">
          ...
          <div class="row g-2 pt-4">
            <div class="col-6">
              <div class="d-flex align-items-center">
                ...
                <span class="ms-2">申请同学：{{item.xm}}</span>
              </div>
            </div>
            <div class="col-6">
              <div class="d-flex align-items-center">
                <i class="icofont-ui-text-chat"></i>
                <span class="ms-2">荣誉：{{item.ry}}</span>
              </div>
            </div>
          </div>
          ...
          <div class="d-flex align-items-center justify-content-between mb-2">
            ...
            <span class="small light-danger-bg p-1 rounded" v-if="item.state==0">
              <i class="icofont-ui-clock"></i>
              {{(item.state==0)?"已提交":(item.state==1?"未通过":"通过")}}
            </span>
            <span class="small light-warning-bg p-1 rounded" v-if="item.state==1">
              <i class="icofont-ui-clock"></i>
              {{(item.state==0)?"已提交":(item.state==1?"未通过":"通过")}}
            </span>
            <span class="small light-success-bg p-1 rounded" v-if="item.state==2">
      ...
      >
        <el-dialog :visible.sync="deletedialogVisible" width="30%" :before-close="handleClose">
          <div class="modal-header">
            <h5 id="deleteprojectLabel">你确定要删除《{{item.ry}}》吗</h5>
          </div>
...
export default {
  ...
  methods: {
    ...
    Mydelete() {
      axios({
        url: "http://47.103.85.74:8087/deleteSign",
        method: "post",
        headers: {
          key: "Content-Type",
