<template>
  ...
  <form class="form">
    ...
    <div class="form__content">
      <h1>Authorization</h1>
      <el-radio-group v-model="stulogin.authority" style="margin:0 10px ">
        <el-radio label="student" style="margin:10px;color:white" class="color-white">我是学生</el-radio>
        <el-radio label="teacher" style="margin:10px;color:white " class="color-white">我是教师</el-radio>
        <el-radio label="admin" style="margin:10px;color:white " class="color-white">我是管理员</el-radio>
      </el-radio-group>
      <div class="styled-input">
        <input type="text" class="styled-input__input" name="nickname" v-model="stulogin.username" />
      ...
      <div class="styled-input">
        <input type="password" class="styled-input__input" v-model="stulogin.password" />
        <div class="styled-input__placeholder">
          <span class="styled-input__placeholder-text">Password</span>
        </div>
        <div class="styled-input__circle"></div>
      </div>
      <button type="button" class="styled-button" v-on:click.prevent="SubmitstuInfo">
  ...
  <!-- <form class="row g-1 p-3 p-md-4">
        <div class="col-12 text-center mb-4">
          <h1>登录</h1>
          <span>欢迎使用学生评奖评优系统</span>
        </div>
        <div class="col-12">
          <div class="mb-3">
            <el-radio-group v-model="stulogin.authority">
              <el-radio label="student" class="color-white">我是学生</el-radio>
              <el-radio label="teacher" class="color-white">我是教师</el-radio>
              <el-radio label="admin" class="color-white">我是管理员</el-radio>
            </el-radio-group>
          </div>
          <div class="mb-2">
...
export default {
      ...
      placeholders.forEach(function(el, i) {
        ...
        for (
          var _iterator = value,
            ...
            _i = 0,
            my_iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();
          ;
          ...
          if (_isArray) {
            if (_i >= my_iterator.length) break;
            _ref = my_iterator[_i++];
          } else {
            _i = my_iterator.next();
            if (_i.done) break;
  ...
  methods: {
    SubmitstuInfo() {
      ...
      axios({
        method: "POST",
        url: "http://47.103.85.74:8080/user",
        headers: {
          key: "Content-Type",
        ...
        if (!response.data.Error) {
          ...
          sessionStorage.setItem("pos", JSON.stringify(response.data.authority));
      ...
      axios({
        method: "POST",
        url: "http://47.103.85.74:8081/stuInfo/authority",
        headers: {
          key: "Content-Type",
          value: "application/json",
          type: "text"
        ...
        data: {
          id:that.stulogin.username
          }
      }).then(response => {
          // sessionStorage.clear();
          sessionStorage.setItem("applyAdd", JSON.stringify(response.data.applyAdd));
          sessionStorage.setItem("auditCommand", JSON.stringify(response.data.auditCommand));
          sessionStorage.setItem("auditPass", JSON.stringify(response.data.auditPass));
          sessionStorage.setItem("award", JSON.stringify(response.data.award));
          sessionStorage.setItem("bulkImport", JSON.stringify(response.data.bulkImport));
          sessionStorage.setItem("courseContestsLoading", JSON.stringify(response.data.courseContestsLoading));
          sessionStorage.setItem("modifyOtherInfo", JSON.stringify(response.data.modifyOtherInfo));
          sessionStorage.setItem("modifyPersonalInfo", JSON.stringify(response.data.modifyPersonalInfo));
          sessionStorage.setItem("readRwardInfo", JSON.stringify(response.data.readRwardInfo));
